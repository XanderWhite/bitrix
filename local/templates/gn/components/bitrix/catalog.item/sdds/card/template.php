...
if (!empty($arParams['PRODUCT_BLOCKS_ORDER'])) {
	foreach ($arParams['PRODUCT_BLOCKS_ORDER'] as $blockName) {
		switch ($blockName) {
			...
			// 				8. Блок торговых предложений (SKU)

			// 				Что делает:

			// Показывает варианты товара (например, разные цвета или размеры).

			// Может отображаться как текст (размеры: S, M, L) или изображения (цвета).
...
// 9. Блок сравнения товаров
// Что делает:

// Добавляет чекбокс для включения товара в список сравнения.
if (
